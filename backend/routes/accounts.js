...
const { scopeUserIds, visibleAccountIds } = require('./scope');
...
router.get('/', async (req, res) => {
  try {
    ...
    // is_owner lets the frontend show owner-only controls (hide toggle); shared accounts are read-only.
    const accounts = await sql`
      SELECT a.*, u.name as owner_name, pi.institution_name, prev.prev_balance,
             (a.user_id = ${req.user.id}) AS is_owner
      FROM accounts a
